...
{
    static class TourFactory
    {

        public static ITour GetTour(string tourType)
        {
            switch (tourType.ToLower())
            {
                case "riding":
                    return new RidingTour();
                case "walking":
                    return new WalkingTour();
                default:
                    return new WalkingTour();
            }
        }
...
//Create an Interface named IVehicle that has a stubbed out public void Drive method. //DONE   ITour and Tour
//Create 2 new public classes that will conform to IVehicle.// DONE ITour, Riding and Walking
